...
data CodeGenState =
    CodeGenState
        ...
        , constantStrings :: Map.Map String Int
        , uniqueConstantStrId :: Int
        }
    deriving (Show)
...
getNewUniqueID = do
    ...
    State.modify' $ \(CodeGenState vs counter lab genf cf si str sid) ->
        CodeGenState vs (counter + 1) lab genf cf si str sid
    return currentCount
...
getNewUniqueLabel = do
    ...
    State.modify' $ \(CodeGenState vs counter lab genf cf si str sid) ->
        CodeGenState vs counter (lab + 1) genf cf si str sid
    let lab = "L" ++ show currentCount
    return lab

getNewUniqueStrID :: CodeGenStateM Int
getNewUniqueStrID = do
    currentCount <- State.gets uniqueConstantStrId
    State.modify' $ \(CodeGenState vs counter lab genf cf si str sid) ->
        CodeGenState vs counter lab genf cf si str (sid + 1)
    return currentCount
    --for each term, (function name, AASM generated, # of var)

aasmGen :: TAST -> [(Ident, Map.Map Ident Type)] -> ([(Ident, ([AAsm], Int))], Map.Map String Int)
  ...
  where
    initialState =
        CodeGenState
            ...
            , constantStrings = Map.empty
            , uniqueConstantStrId = 0
            }
    assemM :: CodeGenStateM ([(Ident, ([AAsm], Int))], Map.Map String Int)
    assemM = do
        _genAAsm <- genTast tast
        CodeGenState _ _ _ genf _ _ strmap _ <- State.get
        return (reverse genf, strmap)
...
genTast (TSeq e1 e2) = do
    ...
    gen2 <- genTast e2
    return $ gen1 ++ gen2
genTast tast@(TAssign (TVIdent x _) expr _b) = do
...
genTast (TAssign lv expr _b) = do
    ...
    if findSize (typeOfLVal lv) sinfo == 4
        then return $ lvgen ++ gen ++ nullcheck ++ [AAsm [APtr $ ATemp n] ANop [ALoc $ ATemp n']]
        else return $ lvgen ++ gen ++ nullcheck ++ [AAsm [APtrq $ ATemp n] ANopq [ALoc $ ATemp n']]
...
genTast (TPtrAssign lv (AsnOp b) e) = do
    ...
    if findSize (typeOfLVal lv) sinfo == 4
        then return $
             lvgen ++ asgnmnt ++ nullchk ++ shiftcheck ++ performOp ++ [AAsm [APtr $ ATemp n] ANop [ALoc $ ATemp n'']]
        else return $
             lvgen ++ asgnmnt ++ nullchk ++ shiftcheck ++ performOp ++ [AAsm [APtr $ ATemp n] ANopq [ALoc $ ATemp n'']]
genTast (TDef fn t e) = do
    let ARROW ts _r = t
        args = map fst ts
        decls = args ++ getDecls e
        v' = Map.fromList $ zip decls [0 ..]
    State.modify' $ \(CodeGenState _vs _counter lab genf _cf si str sid) ->
        CodeGenState v' (length decls) lab genf fn si str sid
    ...
    State.modify' $ \(CodeGenState vs counter lab genf cf si str sid) ->
        CodeGenState vs counter lab ((fn, (funGen, counter)) : genf) cf si str sid
    return funGen
            ...
            case typeOfTExp e of
                STRUCT s ->
                    case Map.lookup s sinfo of
                        ...
                        _ -> error "Malformed. Check type-checker."
                GENSTRUCT ptyp st ->
                    let suff = foldl (\s t -> s ++ "_" ++ show t) "" ptyp
                     in case Map.lookup ("'" ++ st ++ suff) sinfo of
                            Just (_, _, offsets) ->
                                case Map.lookup field offsets of
                                    Just (o, _) -> o
                                    _ -> error "Malformed. Check type-checker."
...
genAddr (TField (TIdent x tp) field _) dest sinfo = do
                    ...
                    case Map.lookup s sinfo of
                        Just (_, _, offsets) ->
                            case Map.lookup field offsets of
                                ...
                                _ -> error "Malformed. Check type-checker."
                        _ -> error "Malformed. Check type-checker."
                GENSTRUCT ptyp struct ->
                    let suff = foldl (\s t -> s ++ "_" ++ show t) "" ptyp
                     in case Map.lookup ("'" ++ struct ++ suff) sinfo of
                            Just (_, _, offsets) ->
                                case Map.lookup field offsets of
                                    Just (o, _) -> o
                                    _ -> error "Malformed. Check type-checker."
    ...
    return $ nullcheck ++ [AAsm [dest] AAddq [ALoc st, AImm offset]]
genAddr (TField e field _) dest sinfo = do
    let (b, boffset) = fieldOffset e sinfo
    gen <- genAddr b dest sinfo
    let addBOffset = [AAsm [dest] AAddq [ALoc dest, AImm boffset]]
        offset =
            case typeOfTExp e of
                STRUCT s ->
                    case Map.lookup s sinfo of
    ...
    return $ gen ++ nullcheck1 ++ addBOffset ++ nullcheck2 ++ [AAsm [dest] AAddq [ALoc dest, AImm offset]]
genAddr (TArrAccess (TIdent a _) (TInt x) typ) dest sinfo = do
    let siz = findSize typ sinfo
    if x > 1073741816 `div` siz
        then return [AControl $ AJump "memerror"]
            ...
            let off = siz * x
            return $ boundcheck ++ [AAsm [dest] AAddq [ALoc arr, AImm off]]
genAddr (TArrAccess arr (TInt x) typ) dest sinfo = do
    ...
    if x > 1073741816 `div` siz
        then return [AControl $ AJump "memerror"]
            ...
            let off = siz * x
            return $ genArr ++ boundcheck ++ [AAsm [dest] AAddq [ALoc $ ATemp n, AImm off]]
genAddr (TArrAccess (TIdent a _) idx typ) dest sinfo = do
    ...
    return $
        genIdx ++
        boundcheck ++ [AAsm [dest] AMulq [AImm sizefact, ALoc $ ATemp n'], AAsm [dest] AAddq [ALoc dest, ALoc arr]]
genAddr (TArrAccess arr idx typ) dest sinfo = do
    ...
    return $
        ...
        genIdx ++
        boundcheck ++
        [AAsm [dest] AMulq [AImm sizefact, ALoc $ ATemp n'], AAsm [dest] AAddq [ALoc dest, ALoc $ ATemp n]]
...
checkBounds arr idx = do
    ...
    l3 <- getNewUniqueLabel
    size <- getNewUniqueID
    let gensize = [AAsm [ATemp size] ASubq [ALoc arr, AImm 8], AAsm [ATemp size] ANop [ALoc $ APtr $ ATemp size]]
        sizechk =
            case idx of
                AImm x
                    | x < 0 -> [AControl $ AJump "memerror"]
                    | otherwise -> [AControl $ ACJump' ALe (ALoc $ ATemp size) idx "memerror" l3, AControl $ ALab l3]
                _ ->
    ...
    case tp of
        ...
        STRUCT a ->
            case Map.lookup a info of
                Just (size, _, _) -> size
                Nothing -> error $ "accessing undeclared struct " ++ a
        GENSTRUCT ptyp a ->
            let suff = foldl (\s t -> s ++ "_" ++ show t) "" ptyp
             in findSize (STRUCT ("'" ++ a ++ suff)) info
        FUNPTR _ _ -> 8
        _ -> error $ "Invalid type to findSize " ++ show tp
...
genExp (TChar a) dest = return [AAsm [dest] ANopb [AImm $ fromIntegral (fromIntegral a :: Int8)]]
genExp (TString st) dest = do
    stringmap <- State.gets constantStrings
    case Map.lookup st stringmap of
        Just i -> return [AAsm [dest] ANopq [AStr st]]
        Nothing -> do
            strid <- getNewUniqueStrID
            State.modify' $ \(CodeGenState vs counter lab genf cf si str sid) ->
                CodeGenState vs counter lab genf cf si (Map.insert st strid str) sid
            return [AAsm [dest] ANopq [AStr st]]
genExp expr@(TBinop binop exp1 exp2) dest
    ...
    | binop == Sal || binop == Sar = do
        ...
        return $
            ...
            checker ++
            [AControl $ ALab l1] ++
            combine ++ [AControl $ AJump l3, AControl $ ALab l2] ++ failure ++ [AControl $ AJump l3, AControl $ ALab l3]
...
genExp (TArrAccess (TIdent a _) exp2 tp) dest = do
    ...
    info <- State.gets structInfo
    let sizefact = findSize tp info
        offset = [AAsm [dest] AMulq [AImm sizefact, ALoc $ ATemp n'], AAsm [dest] AAddq [ALoc dest, ALoc arr]]
...
genExp (TArrAccess exp1 exp2 tp) dest = do
    ...
    info <- State.gets structInfo
    let sizefact = findSize tp info
        offset = [AAsm [dest] AMulq [AImm sizefact, ALoc $ ATemp n'], AAsm [dest] AAddq [ALoc dest, ALoc $ ATemp n]]
...
genExp (TRefFun fn) dest = return [AAsm [dest] ALeaq [AFunPtr fn]]
genExp (TRefFunAp fn args typ) dest = do
    allocmap <- State.gets variables
    let fptr = ATemp $ allocmap Map.! fn
        argLen = length args
    ids <- replicateM argLen getNewUniqueID
    let tmpVars = map ATemp ids
    gens <- mapM (\(i, e) -> genExp e (tmpVars !! i)) $ zip [0 ..] args
    ...
    let (inReg, inStk) = splitAt 6 tmpVars
        movArg = map (\(i, tmp) -> AAsm [argRegs !! i] ANopq [ALoc tmp]) $ zip [0 ..] inReg
        assign = assignType typ (AReg 0) dest
    return $ gen ++ movArg ++ [ACallRef fptr inStk (length inReg)] ++ assign
...
assignType tp src dest =
    case tp of
        ...
        STRING -> [AAsm [dest] ANopq [ALoc src]]
        CHAR -> [AAsm [dest] ANopb [ALoc src]]
        STRUCT _ -> error "Invalid assignment of large type (struct)"
        GENSTRUCT _ _ -> error "Invalid assignment of large type (struct)"
        FUNPTR _ _ -> [AAsm [dest] ANopq [ALoc src]]
...
assignHeap tp src dest =
    case tp of
        ...
        STRING -> [AAsm [dest] ANopq [ALoc $ APtrq src]]
        CHAR -> [AAsm [dest] ANopb [ALoc $ APtrb src]]
        STRUCT _ -> error "Invalid assignment of large type (struct)"
        GENSTRUCT _ _ -> error "Invalid assignment of large type (struct)"
        FUNPTR _ _ -> [AAsm [dest] ANopq [ALoc $ APtrq src]]
