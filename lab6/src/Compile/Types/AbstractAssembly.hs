...
module Compile.Types.AbstractAssembly where
  ...
  instance Show AAsm where
    ...
    show (AAsm [dest] ANopb [src]) = "\t" ++ show dest ++ " <-- " ++ show src ++ "\n"
    ...
    show (ACallRef f xs argNum) = "\tCall " ++ show f ++ " with parameters on stack: " ++ show xs ++ ", use " ++ show argNum ++ " registers\n"
