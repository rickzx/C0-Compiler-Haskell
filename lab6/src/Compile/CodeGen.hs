...
codeGen tast structs =
    let 
        (tastGen, _strmap) = aasmGen tast structs
        memerr = [
...
asmGen tast header structs =
    let memerr = ".memerror:\n\tmovl $12, %edi\n\txorl %eax, %eax\n\tcall raise\n"
        (tastGen, strmap) = aasmGen tast structs
        constStr = Map.foldlWithKey' (\accum str idx -> accum ++ "\tstr" ++ show idx ++ ": .string \"" ++ str ++ "\"\n") ".DATA\n" strmap
        globs = map (\(x, _) -> if x == "a bort" then Global "_c0_abort_local411" else Global $ "_c0_" ++ x) tastGen
        globString = concatMap (\line -> show line ++ "\n") globs
     in globString ++ constStr ++ concatMap (\(fn, (aasms, lv)) -> generateFunc (fn, aasms, lv) header strmap) tastGen ++ memerr

--add in string map for generating assembly
generateFunc :: (String, [AAsm], Int) -> Header -> Map.Map String Int -> String
generateFunc (fn, aasms, localVar) header stringmap =
    let (coloring, stackVar, calleeSaved) =
        ...
        -- (trace $ show fn ++ "\n" ++ show coloring ++ "\n\n" ++ show aasms)
        insts = concatMap (\x -> List.filter nonTrivial (toAsm x coloring header stringmap)) aasms
