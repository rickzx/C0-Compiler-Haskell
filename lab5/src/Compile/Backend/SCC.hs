...
scc stmts varwork blockwork vmap usemap (varmap, blockmap) 
    ...
    where 
        ...
        foldphiargs varmap = foldr (\x acc -> 
            ...
            in
                if 
                xval == N || acc == P || acc == N || acc == xval then
                acc else xval) N 
            ...
            case x of 
                        ...
                        in
                            case l of 
                                ...
                                _ -> let
                                    ...
                                    in
                                        traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
                AAsmS _ (AAsm [dest] op [arg1, arg2])
                    | isTemp dest &&
                                   (op == AAdd ||
                                    op == AAddq ||
                                    op == ASub ||
                    ...
                    in
                        case (l, r) of 
                            ...
                            _-> let
                                ...
                                in
                                    traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
                AAsmS _ (AControl (ACJump v l1 l2)) ->
                    let 
                ...
                AAsmS _ (AControl (AJump l1)) -> 
                    ...
                    in 
                        ...
                        traverseBlock xs flag (varmap, blockmap) (defmap, usemap) (varwork, blockwork)
                AAsmS _ (AControl (ACJump' op arg1 arg2 l1 l2)) -> let          
                        usemap' = if flag then usemap else insertUse x usemap [arg1, arg2]
                        l = varlookup arg1 varmap
                        r = varlookup arg2 varmap
                        (visited1, visited2) = (fromMaybe False (Map.lookup l1 blockmap), fromMaybe False (Map.lookup l2 blockmap))
                            ...
                            _ -> case (visited1, visited2) of 
                                    ...
                                    (False, True) -> let 
                                        ...
                                        in 
                                            traverseBlock xs flag (varmap, nblockmap) (defmap, usemap') (varwork, nblockwork)
                                    _ -> traverseBlock xs flag (varmap, blockmap) (defmap, usemap') (varwork, blockwork)
                AAsmS _ (ARel [dest] binop [arg1, arg2])
                    | isTemp dest -> 
                        let 
                            usemap' = if flag then usemap else insertUse x usemap [arg1, arg2]
                            defmap' = if flag then defmap else Map.insert dest x defmap
...
changeStmt stmt loc c modset = 
    ...
    in
        case stmt' of 
            PhiS lno (dest, args) ->  Map.insert lno (PhiS lno (dest, map(\x -> if x == ALoc(loc) then (AImm c) else x) args)) modset
            AAsmS lno (AAsm [dest] op args) ->
                Map.insert lno (AAsmS lno (AAsm [dest] op (map(\x -> if x == ALoc(loc) then (AImm c) else x) args))) modset
            AAsmS lno (ARel [dest] op args) ->
                 Map.insert lno (AAsmS lno (ARel [dest] op (map(\x -> if x == ALoc(loc) then (AImm c) else x) args))) modset
            AAsmS lno (ARet arg) -> 
                Map.insert lno (AAsmS lno (ARet $ AImm c)) modset
            AAsmS lno (ACall lab args i) -> 
                Map.insert lno (AAsmS lno (ACall lab (map(\x -> if x == ALoc(loc) then (AImm c) else x) args) i)) modset
            AAsmS lno (AControl (ACJump arg l1 l2)) -> 
                if c /= 0 then Map.insert lno (AAsmS lno (AControl $ AJump l1)) modset 
                else Map.insert lno (AAsmS lno (AControl $ AJump l2)) modset
            AAsmS lno (AControl (ACJump' op arg1 arg2 l1 l2)) -> let
                    ...
                    case stmt' of 
                        ...
                        AAsmS lno (AControl (ACJump' op arg1 arg2 l1 l2))
                            | arg1 == arg2 ->
                                ...
                                Map.insert lno (AAsmS lno (AControl $ AJump l2)) modset
                            | arg1 == ALoc loc -> 
                                Map.insert lno
                                (AAsmS lno (AControl (ACJump' op (AImm c) arg2 l1 l2)))
                                modset
                            | otherwise ->
                                Map.insert lno
                                (AAsmS lno (AControl (ACJump' op arg1 (AImm c) l1 l2)))
                                modset
                        _ -> modset
            _ -> modset
...
changeStmtDebug stmt loc c modset = 
    ...
    in
        case stmt' of 
            PhiS lno (dest, args) -> (trace $
                "change stmt:" ++ show loc ++ "to" ++ show c ++ "\n" ++ show stmt) Map.insert lno (PhiS lno (dest, map(\x -> if x == ALoc(loc) then (AImm c) else x) args)) modset
            AAsmS lno (AAsm [dest] op args) ->(trace $
                "change stmt:" ++ show loc ++ "to" ++ show c ++ "\n" ++ show stmt)
                Map.insert lno (AAsmS lno (AAsm [dest] op (map(\x -> if x == ALoc(loc) then (AImm c) else x) args))) modset
            AAsmS lno (ARel [dest] op args) ->
                (trace $
                "change stmt:" ++ show loc ++ "to" ++ show c ++ "\n" ++ show stmt) Map.insert lno (AAsmS lno (ARel [dest] op (map(\x -> if x == ALoc(loc) then (AImm c) else x) args))) modset
            AAsmS lno (ARet arg) -> 
                (trace $
                "change stmt:" ++ show loc ++ "to" ++ show c ++ "\n" ++ show stmt)Map.insert lno (AAsmS lno (ARet $ AImm c)) modset
            AAsmS lno (ACall lab args i) -> 
                (trace $
                "change stmt:" ++ show loc ++ "to" ++ show c ++ "\n" ++ show stmt)Map.insert lno (AAsmS lno (ACall lab (map(\x -> if x == ALoc(loc) then (AImm c) else x) args) i)) modset
            AAsmS lno (AControl (ACJump arg l1 l2)) -> 
                if c /= 0 then (trace $
