...
runSCC blks g pre fn =
    let (eblk, _, blockmap) = enumBlock blks
     ...
     in (bblk, allVars, newg, newpre)

--enumerate the lines of AASM with in  blocks + initialize all the block list
enumBlock :: Map.Map Ident Block -> (Map.Map Ident [StmtS], Int, Map.Map Ident Bool)
enumBlock =
    Map.foldrWithKey'
...
scc stmts varwork blockwork vmap usemap (varmap, blockmap)
    ...
    | not (null blockwork)
     ...
     =
        let blk = Set.elemAt 0 blockwork
         ...
         in scc stmts varwork' blockwork' defmap' usemap' (varmap', blockmap')
    ...
    | otherwise =
        let fstvar = Set.elemAt 0 varwork
  ...
  where
    ...
    varlookup v varmap =
        case v of
            ...
            ALoc v' ->
                if isTemp v'
                    then fromMaybe N (Map.lookup v' varmap)
                    else P
    foldphiargs varmap =
        foldr
            (\x acc ->
                 let xval = varlookup x varmap
                  in if xval == N || acc == P || acc == N || acc == xval
                         then acc
                         else xval)
            N
    changeInVal dest value (varmap, varwork) =
        let currVal = fromMaybe N (Map.lookup dest varmap)
         in if currVal /= value
                then (Map.insert dest value varmap, Set.insert dest varwork)
                else (varmap, varwork)
    ...
    traverseBlock ::
        ...
        -> ( Map.Map ALoc Value
           ...
           , Set.Set ALoc
           , Set.Set Ident)
    traverseBlock [] flag (varmap, blockmap) (defmap, usemap) (varwork, blockwork) =
        (varmap, blockmap, defmap, usemap, varwork, blockwork)
        ...
        case x of
            PhiS _ _ (dest, args) ->
                let usemap' =
                        if flag
                            then usemap
                            else insertUse x usemap args
                    defmap' =
                        if flag
                            then defmap
                            else Map.insert dest x defmap
                    res = foldphiargs varmap args
                 in case res of
                        Const c1 ->
                            let (nvarmap, nvarwork) = changeInVal dest (Const c1) (varmap, varwork)
                             in traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
                        other ->
                            let (nvarmap, nvarwork) = changeInVal dest other (varmap, varwork)
                             in traverseBlock xs flag (varmap, blockmap) (defmap', usemap') (varwork, blockwork)
            AAsmS _ _ (AAsm [dest] ANop [c])
                | isTemp dest ->
                    let usemap' =
                            if flag
                                then usemap
                                else insertUse x usemap [c]
                        defmap' =
                            if flag
                                then defmap
                                else Map.insert dest x defmap
                     in case c of
                            ALoc (AReg _) ->
                                let (nvarmap, nvarwork) = changeInVal dest P (varmap, varwork)
                                 in traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
                            AImm i ->
                                let (nvarmap, nvarwork) = changeInVal dest (Const i) (varmap, varwork)
                                 in traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
                            ALoc l ->
                                if isTemp l
                                    then let (nvarmap, nvarwork) =
                                                 changeInVal dest (fromMaybe N (Map.lookup l varmap)) (varmap, varwork)
                                          in traverseBlock
                                                 xs
                                                 flag
                                                 (nvarmap, blockmap)
                                                 (defmap', usemap')
                                                 (nvarwork, blockwork)
                                    else let (nvarmap, nvarwork) = changeInVal dest P (varmap, varwork)
                                          in traverseBlock
                                                 xs
                                                 flag
                                                 (nvarmap, blockmap)
                     ...
                     in case l of
                            Const c1 ->
                                let c1' = fromIntegral c1 :: Int32
                                    ...
                                    (nvarmap, nvarwork) =
                                        changeInVal dest (Const (fromIntegral res :: Int)) (varmap, varwork)
                                 in traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
                            _ ->
                                let (nvarmap, nvarwork) = changeInVal dest P (varmap, varwork)
                                 in traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
            AAsmS _ _ (AAsm [dest] op [arg1, arg2])
                | isTemp dest &&
                      (op == AAdd ||
                       op == AAddq ||
                       op == ASub ||
                       op == ASubq || op == AMul || op == ABAnd || op == ALAnd || op == ABOr || op == ALOr || op == AXor) ->
                    let usemap' =
                            if flag
                                then usemap
                                else insertUse x usemap [arg1, arg2]
                        defmap' =
                            if flag
                                then defmap
                                else Map.insert dest x defmap
                                ...
                                let c1' = fromIntegral c1 :: Int32
                                    ...
                                    foldop =
                                        case op of
                                            ...
                                            AXor -> c1' `xor` c2'
                                    (nvarmap, nvarwork) =
                                        changeInVal dest (Const (fromIntegral foldop :: Int)) (varmap, varwork)
                                 in traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
                            _ ->
                                let (nvarmap, nvarwork) = changeInVal dest P (varmap, varwork)
                                 in traverseBlock xs flag (nvarmap, blockmap) (defmap', usemap') (nvarwork, blockwork)
            AAsmS _ _ (AControl (ACJump v l1 l2)) ->
                let usemap' =
                        if flag
                            then usemap
                            else insertUse x usemap [v]
                    l = varlookup v varmap
                    (visited1, visited2) =
                        (fromMaybe False (Map.lookup l1 blockmap), fromMaybe False (Map.lookup l2 blockmap))
                 in case l of
                        Const 1 ->
                            if not visited1
                                then let nblockmap = Map.insert l1 True blockmap
                        ...
                        Const 0 ->
                            if not visited2
                                then let nblockmap = Map.insert l2 True blockmap
                        ...
                        _ ->
                            case (visited1, visited2) of
                                (False, False) ->
                                    let nblockmap = Map.insert l1 True (Map.insert l2 True blockmap)
                                     ...
                                     in traverseBlock
                                            ...
                                            (varwork, nblockwork)
                                (True, False) ->
                                    let nblockmap = Map.insert l2 True blockmap
                                        nblockwork = Set.insert l2 blockwork
                                     in traverseBlock
                                            xs
                                            flag
                                            ...
                                            (varwork, nblockwork)
                                (False, True) ->
                                    let nblockmap = Map.insert l1 True blockmap
                                        nblockwork = Set.insert l1 blockwork
                                     in traverseBlock
                                            xs
                                            flag
            ...
            AAsmS _ _ (AControl (AJump l1)) ->
                if not (fromMaybe False (Map.lookup l1 blockmap))
                    then let nblockmap = Map.insert l1 True blockmap
                             nblockwork = Set.insert l1 blockwork
                          in traverseBlock xs flag (varmap, nblockmap) (defmap, usemap) (varwork, nblockwork)
                    else traverseBlock xs flag (varmap, blockmap) (defmap, usemap) (varwork, blockwork)
            AAsmS _ _ (AControl (ACJump' op arg1 arg2 l1 l2)) ->
                let usemap' =
                        if flag
                            then usemap
                            else insertUse x usemap [arg1, arg2]
            ...
            _ ->
                if flag
                    then traverseBlock xs flag (varmap, blockmap) (defmap, usemap) (varwork, blockwork)
                    else let (dfnst, usest) = getDefUse x
...
changeStmt stmt loc c modset =
    let stmt' = fromMaybe stmt (Map.lookup (getLineNum stmt) modset)
     in case stmt' of
            PhiS lno pb (dest, args) ->
                Map.insert
                    lno
                    (PhiS
                         lno
                         ...
                         , map (\x ->
                               ...
                               args))
                    modset
            AAsmS lno pb (AAsm [dest] op args) ->
                Map.insert
                    lno
                    (AAsmS
                         lno
                         ...
                         (AAsm
                              ...
                              (map (\x ->
                                   ...
                                   args)))
                    modset
            AAsmS lno pb (ARel [dest] op args) ->
                Map.insert
                    lno
                    (AAsmS
                         lno
                         ...
                         (ARel
                              ...
                              (map (\x ->
                                   ...
                                   args)))
                    modset
            AAsmS lno pb (ARet arg) -> Map.insert lno (AAsmS lno pb (ARet $ AImm c)) modset
            AAsmS lno pb (ACall lab args i) ->
                Map.insert
                    lno
                    (AAsmS
                         lno
                         ...
                         (ACall
                              ...
                              i))
                    modset
            AAsmS lno pb (AControl (ACJump arg l1 l2)) ->
                if c /= 0
                        ...
                        AAsmS lno pb (AControl (ACJump' op arg1 arg2 l1 l2))
                            ...
                            | arg1 == ALoc loc ->
                                (trace $ "change stmt: " ++ show loc ++ " to " ++ show c ++ "\n" ++ show stmt)
                                    Map.insert
                                    lno
                                    (AAsmS lno pb (AControl (ACJump' op (AImm c) arg2 l1 l2)))
                                    modset
                            | otherwise ->
                                (trace $ "change stmt:" ++ show loc ++ "to" ++ show c ++ "\n" ++ show stmt)
                                    Map.insert
                                    lno
                                    (AAsmS lno pb (AControl (ACJump' op arg1 (AImm c) l1 l2)))
                                    modset
