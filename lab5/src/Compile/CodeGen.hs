    ...
    generateFunc (fn, aasms, localVar) header trdict =
        let
            ...
            trelim = case Map.lookup fn trdict of
                Just _ -> hd ++ elim
                Nothing -> elim
            (coloring, stackVar, calleeSaved) = if length allVars >= 3000 then allStackColor allVars else
            ...
            optinsts = remove_move insts
                    where
                        ...
                        remove_move (x:y:xs) = case (x, y) of
                            (Movq op1 op2, _) -> if op1 == op2 then remove_move (y: xs) else x: remove_move(y:xs)
